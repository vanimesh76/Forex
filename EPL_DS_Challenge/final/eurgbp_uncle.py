    ...
    while True:
        ...
        if df.iloc[-2].close != cl or df.iloc[-2].open != op:
            if df.iloc[-2].smaC < df.iloc[-2].smaO and check == 0:
                result_sell = Action(symbol, lot, 0)  #SELL Action
                if checks == 1 and buy_up == 0:
                    result_buy = Action_close(result_buy.order, symbol, 1, lot)     #Action_close
                    print(f"Close  Symbol-->{symbol} ||| result_comment-->{result_buy.comment}")
                    if result_buy.comment == "Requote":
                        result_buy = Action_close(result_buy.order, symbol, 1, lot)
            ...
            if df.iloc[-2].smaC > df.iloc[-2].smaO and checks == 0:
                result_buy = Action(symbol, lot, 1)  #BUY Action

                if check == 1 and sell_up == 0:
                    result_sell = Action_close(result_sell.order, symbol, 0, lot)   #Action_close
                    print(f"Close  Symbol-->{symbol} ||| result_comment-->{result_sell.comment}")
                    if result_sell.comment == "Requote":
                        result_sell = Action_close(result_sell.order, symbol, 0, lot)
        ...
        if checks == 1 and buy_up == 0:
            ...
            pp = price_action(symbol, 0.02, result_buy.price, sell_price,mt5.ORDER_TYPE_BUY)
            if pp > 1.0:
                result_buy = Action_close(result_buy.order, symbol, 1, lot)     #Action_close
                print(f"Close  Symbol-->{symbol} ||| result_comment-->{result_buy.comment}")
                if result_buy.comment == "Requote":
        ...
        if check == 1 and sell_up == 0:
            sell_price = df.iloc[-1].close
            pp = price_action(symbol, 0.02, result_sell.price, sell_price,mt5.ORDER_TYPE_SELL)
            if pp >= 1.0:
                result_sell = Action_close(result_sell.order, symbol, 0, lot)
                print(f"Close  Symbol-->{symbol} ||| result_comment-->{result_sell.comment}")
                if result_sell.comment == "Requote":
