    ...
    while True:
        try:
            ...
            if profit >= 0.30:
                ...
                break
            elif rates_frame.iloc[-2].close != dma:
                Action_close(ticket_no, symbol, signal)
                break
            # elif profit <= -0.50:
            #     Action_close(ticket_no, symbol, signal)
...
global dictt
key = 0
dictt = {'EURUSD':[0, 0, 0.01, 'dma'], 'GBPUSD':[0, 0, 0.01, 'dma'], 'USDJPY':[0, 0, 0.01, 'dma'], \
'EURGBP':[0, 0, 0.01, 'dma'],'USDCHF':[0, 0, 0.01, 'dma'], 'CADJPY':[0, 0, 0.03, 'dma']}
...
while True:
    for symbol in dictt:
        ...
        if rates_frame.iloc[-2].close < sett and \
            rates_frame.iloc[-1].close > sett and \
            ...
            p1 = threading.Thread(target=Profit_checker, args=(result.order, symbol, signal, rates_frame.iloc[-2].close))
            p1.start()
            dictt[symbol][0] = 1
