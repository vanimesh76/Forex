...
def get_values(symbol):
    ...
    rates_frame['sma']= rates_frame['close'].rolling(window=13).mean()
    ...
    while True:
        if hour_passed:
            ...
            if a.iloc[-2].name.hour == 0:
                y1,c = modf(round(a.iloc[-4].sma, 6))
                y2, c = modf(round(a.iloc[-2].sma, 6))
        ...
        if sell_check == 1:
            if mt5.positions_get(ticket=result_sell.order)[0].profit >= 0.90:
        ...
        if buy_check == 1:
            if mt5.positions_get(ticket=result_buy.order)[0].profit >= 0.90:
