...
global dictt
key = 0
dictt = {'EURUSD':[0, 0, 0.01], 'GBPUSD':[0, 0, 0.01], 'USDJPY':[0, 0, 0.01], 'EURGBP':[0, 0, 0.01],'USDCHF':[0, 0, 0.01], 'CADJPY':[0, 0, 0.03]}
while True:
    for symbol in dictt:
        rates_frame = get_values(symbol)
        lot = dictt[symbol][2]
        ...
        if rates_frame.iloc[-2].close > rates_frame.iloc[-1].dma and \
         rates_frame.iloc[-3].close < rates_frame.iloc[-1].dma and \
         ...
         #rates_frame.iloc[-1].close > rates_frame.iloc[-2].close:
            ...
            if buy_profit > -0.40:
                ...
                p1 = threading.Thread(target=Profit_checker, args=(result.order, symbol, signal, rates_frame.iloc[-1].dma))
                p1.start()
                dictt[symbol][0] = 1
                dictt[symbol][1] = 0
            
        elif rates_frame.iloc[-2].close < rates_frame.iloc[-1].dma and \
        rates_frame.iloc[-3].close > rates_frame.iloc[-1].dma and \
